...
<body>
    ...
    <script type="text/javascript">
		...
		function productList(type,page,cate1,cate2,sort,target) {
		...
		$(document).ready(function () {
			...
			$('.sortNav input').click(function(){
				...
				if(sizeArr.indexOf("0") != -1) {								// 사이즈 미선택시
					productList(id,pageNum,category1,category2,sort); 
				} else {														// 사이즈가 선택되어있을시
					productList(id,pageNum,category1,category2,sort,"sizeArr"); // 그대로 배열 넘김
				}  
			});
			...
			$('.card-body').on('click','a',function(){
				...
				$("#sizeList").attr("data-cate2", category2);
				$('#sizeList').find('input').prop("checked", false); //체크박스 초기화
			});

			$('#sizeList').on('click','input',function(){
				var category1 = $(this).closest("#sizeList").attr("data-cate1");
				var category2 = $(this).closest("#sizeList").attr("data-cate2");
				/* 히든 처리돼있음
				var pageNum = $("#pageNum").val(); //페이지 넘버
				var category1 = $("#category1").val(); //카테고리1
				var category2 = $("#category2").val(); //카테고리2
				*/
				productList('','1',category1,category2,0,$(this));        
			});
		});
